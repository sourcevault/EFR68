class Dual1 {
	...
	__New(settings=false) {
		Dual1.override(this.settings, settings)
	}
	...
	comboKey(remappingKey=false, combinators=false) {
		...
		this.combo(Dual1.cleanKey(A_ThisHotkey))

		key := remappingKey ? remappingKey : A_ThisHotkey
		Dual1.sendSubKeySet(key, combinators)
	}
	...
	getKeysFor(currentKey, downKey, upKey, settings, combinators) {
		...
		cleanKey := Dual1.cleanKey(currentKey)

		if (this.keys[cleanKey]) {
			keys := this.keys[cleanKey]
		} else {
			keys := new Dual1.KeyPair(downKey, upKey, this.settings, settings, combinators)
	...
	class KeyPair {
		__New(downKey, upKey, defaults, settings, combinators) {
			this.downKey := new Dual1.Key(downKey)
			this.upKey   := new Dual1.Key(upKey, combinators)

			; dmp(this.upKey)

			Dual1.override(defaults, settings, {onto: this})
		...
		getDelay(key) {
			for index, specificDelay in this._specificDelays {
				if (Dual1.contains(specificDelay.keySet, key)) {
	...
	class Key {
		...
		send() {
			...
			Dual1.sendSubKeySet(this.key, this.combinators) ; (*)
		}
		...
		_timeDown := false
		timeDown() {
			return Dual1.timeSince(this._timeDown)
		}

		_lastUpTime := false
		timeSinceLastUp() {
			return Dual1.timeSince(this._lastUpTime)
		}
	}
	...
	keydown(keys, currentKey, lastKey) {
		...
		if (timeSinceLastUp != false
			...
			and Dual1.cleanKey(lastKey) == Dual1.cleanKey(currentKey)) { ; (*2)
	...
	keyup(keys, currentKey, lastKey) {
		...
		if (not downKey.combo
			...
			shorterTimeDownKeys := this.combo(Dual1.cleanKey(currentKey), downKeyTimeDown)
	...
	sendInternal(string) {
		if (Dual1.sentKeys) {
			Dual1.sentKeys.Insert(string)
		} else {

			ec := SubStr(string,0)
			sc := SubStr(string,1,1)
	...
	subKeySet(key) {
		...
		for index, subKey in key {
			key[index] := Dual1.cleanKey(subKey)
		}
